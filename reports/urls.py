...
urlpatterns = patterns('',
	...
	url(r'^submit/upload$',      login_required(submit_upload),                    name='submit-public'),
	url(r'^(?P<pk>\d+)/view$',   login_required(ReportView.as_view()),             name='view'),
	url(r'^(?P<pk>\d+)/verify$', login_required(ReportVerifyView.as_view()),       name='verify'),
	url(r'^(?P<pk>\d+)/close$',  login_required(ReportCloseView.as_view()),        name='close'),
